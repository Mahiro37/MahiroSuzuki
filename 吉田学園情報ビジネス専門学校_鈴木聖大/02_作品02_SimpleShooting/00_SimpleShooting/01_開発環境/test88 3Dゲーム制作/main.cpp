...
#define WINDOW_NAME "Simple Shooting"	// ウインドウの名前(キャプション)
...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	// レンダーステートの設定
	g_pD3DDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);				// 裏面(左回り)をカリング
...
{
	// バッファ＆Zバッファのクリア
	g_pD3DDevice->Clear(0, NULL, (D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER), D3DCOLOR_RGBA(170, 180, 170, 255), 1.0f, 0);
	// Direct3Dによる描画の開始
...
void SetMode(MODE mode)
{
	...
	switch (g_mode)
	{
	case MODE_TITLE:		//タイトルの初期化処理
		InitTitle();
		break;

	case MODE_TUTORIAL:		//チュートリアルの初期化処理
		InitTutorial();
		break;
